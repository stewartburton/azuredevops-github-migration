...
class AzureDevOpsClient:
    ...
    def validate_credentials(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"[OK] Azure DevOps credentials valid. Found {len(projects)} projects."
            )
            return True
        except requests.exceptions.HTTPError as e:
            if e.response.status_code == 401:
                raise AuthenticationError(
                    "Invalid Azure DevOps PAT or insufficient permissions"
                )
            elif e.response.status_code == 404:
                raise AuthenticationError(
                    f"Organization '{self.organization}' not found or no access"
                )
            raise
    ...
    def get_repositories(self, project_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.debug(
                f"Found {len(repos)} repositories in project '{project_name}'"
            )
            return repos
        except requests.exceptions.RequestException as e:
            self.logger.error(
                f"Error getting repositories for project '{project_name}': {str(e)}"
            )
    ...
    def get_repository_size(self, project_name: str, repo_id: str) -> int:
        ...
        try:
            ...
            total_commits = sum(
                branch.get("aheadCount", 0) + branch.get("behindCount", 0)
                for branch in stats
            )
    ...
    def get_repository_branches(
        self, project_name: str, repo_id: str
    ) -> List[Dict[str, Any]]:
    ...
    def get_pipelines(self, project_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.debug(
                f"Found {len(pipelines)} pipelines in project '{project_name}'"
            )
            return pipelines
        except requests.exceptions.RequestException as e:
            self.logger.warning(
                f"Could not get pipelines for project '{project_name}': {str(e)}"
            )
            return []
    ...
    def get_work_items(
        self, project_name: str, wiql_query: str = None
    ) -> List[Dict[str, Any]]:
    ...
    def get_pull_requests(
        self, project_name: str, repository_id: str
    ) -> List[Dict[str, Any]]:
    ...
    def export_repository_data(
        ...
        pipeline_scope: str = "project",
        exclude_disabled_pipelines: bool = False,
    ) -> Dict[str, Any]:
        ...
        """
        self.logger.info(
            f"Exporting data for repository '{repo_name}' in project '{project_name}'"
        )
        ...
        if not repo:
            raise ValueError(
                f"Repository '{repo_name}' not found in project '{project_name}'"
            )
        ...
        if exclude_disabled_pipelines and pipelines:
            ...
            pipelines = [
                p
                for p in pipelines
                if str(p.get("queueStatus", "")).lower() not in {"disabled", "paused"}
            ]
            self.logger.debug(
                f"Filtered disabled pipelines: {before} -> {len(pipelines)}"
            )
        ...
        self.logger.info(
            f"Repository data exported: {len(repo_data['branches'])} branches, "
...
class GitHubClient:
    ...
    def __init__(
        self, token: str, organization: str = None, logger: logging.Logger = None
    ):
        self.token = token
        self.organization = organization
    ...
    def create_repository(
        ...
        if any(c.isspace() for c in name):
            ...
            if original_name != name:
                self.logger.warning(
                    f"Repository name contained whitespace. Auto-normalized '{original_name}' -> '{name}'"
                )
        ...
        try:
            ...
            if response.status_code == 403:
                ...
                if self.repository_exists(name):
                    ...
                    if existing:
                        self.logger.warning(
                            f"[WARN] 403 Forbidden on create, but repository '{name}' exists. Reusing existing repository."
                        )
                        return existing
            ...
            if e.response.status_code == 403:
                raise MigrationError(
                    "Forbidden: insufficient permissions or SSO authorization required for repository creation"
                ) from e
            raise
    ...
    def create_milestone(
        self, repo_name: str, title: str, description: str = ""
    ) -> Dict[str, Any]:
...
class GitMigrator:
    ...
    def migrate_repository_git_history(
        self,
        project_name: str,
        repo_name: str,
        ...
        try:
            ...
            if not clone_url:
                raise GitOperationError(
                    f"No clone URL found for repository '{repo_name}'"
                )
            ...
            gh_org = (
                ...
                or self.github_client.get_user().get("login", "unknown")
            )
            ...
            authenticated_url = self._add_auth_to_url(
                clone_url, "", self.azure_client.pat
            )
            ...
            if not github_repo:
                raise MigrationError(
                    f"GitHub repository '{github_repo_name}' not found"
                )

            github_clone_url = github_repo["clone_url"]
            authenticated_github_url = self._add_auth_to_url(
                github_clone_url, self.github_client.token, ""
            )
            ...
            self._verify_migration(
                temp_dir,
                authenticated_github_url if verify_remote else None,
                verify_remote,
            )
            ...
            raise GitOperationError(
                "Git operation timed out. Repository may be too large."
            )
        except Exception as e:
    ...
    def _verify_migration(
        self, local_repo_path: str, github_url: Optional[str], verify_remote: bool
    ):
        ...
        try:
            ...
            local_branches_list = [
                b.strip() for b in local_branches_cmd.stdout.split("\n") if b.strip()
            ]
            ...
            commit_count = (
                ...
                if commit_result.stdout.strip().isdigit()
                else 0
            )
            ...
            if verify_remote and github_url:
                ...
                if (
                    "://" in display_url
                    and "@" in display_url.split("://", 1)[1].split("/", 1)[0]
                ):
                    # Remove userinfo
                ...
                try:
                    ...
                    remote_branch_lines = [
                        ln for ln in remote_heads.stdout.split("\n") if ln.strip()
                    ]
                    remote_branches_list = [
                        ln.split("\t")[1].replace("refs/heads/", "")
                        for ln in remote_branch_lines
                        if "\trefs/heads/" in ln
                    ]
                    remote_branches = len(remote_branches_list)
                    missing_remote = sorted(
                        set(local_branches_list) - set(remote_branches_list)
                    )
                    missing_local = sorted(
                        set(remote_branches_list) - set(local_branches_list)
                    )
                    branch_match = (
                        not missing_remote
                        and not missing_local
                        and (local_branches == remote_branches)
                    )
                    self.logger.info(
                    ...
                    if missing_remote:
                        self.logger.warning(
                            f"Branches missing on remote: {missing_remote}"
                        )
                    if missing_local:
                        self.logger.warning(
                            f"Extra branches on remote: {missing_local}"
                        )
                    remote_details = {
    ...
    def cleanup(self):
        ...
        for temp_dir in self.temp_dirs:
            try:
                ...
                self.logger.warning(
                    f"Failed to cleanup temp directory {temp_dir}: {str(e)}"
                )
...
class PipelineConverter:
    ...
    def convert_pipelines_to_actions(
        self, pipelines: List[Dict[str, Any]], output_dir: str, dry_run: bool = False
    ) -> List[str]:
        ...
        for i, pipeline in enumerate(pipelines):
            try:
                ...
                if dry_run:
                    ...
                    before = raw_name
                    workflow_stem = self._naming_module.normalize_workflow_stem(
                        before, self._seen_stems, self._naming_config
                    )
                    if workflow_stem != before:
                        # Determine if this is a collision-originated change (numeric suffix present & base appears earlier)
                        collision = False
                        base_candidate = (
                            workflow_stem.rsplit("-", 1)[0]
                            if "-" in workflow_stem
                            else None
                        )
                        if base_candidate and any(
                            s == base_candidate for s in self._seen_stems
                        ):
                            collision = True
                        if collision:
                            self.logger.info(
                                f"Name collision: '{before}' normalized base already used, assigned '{workflow_stem}.yml'"
                            )
                        else:
                            self.logger.debug(
                                f"Normalized pipeline name '{before}' -> '{workflow_stem}.yml'"
                            )
                else:
                ...
                self.logger.info(
                    f"Converted pipeline '{pipeline.get('name')}' to '{workflow_file}'"
                )

            except Exception as e:
                self.logger.error(
                    f"Failed to convert pipeline '{pipeline.get('name')}': {str(e)}"
                )
...
class MigrationOrchestrator:
    ...
    def __init__(self, config_file: str = "migration_config.yaml"):
        ...
        self.git_migrator = GitMigrator(
            self.azure_client, self.github_client, self.logger
        )
    ...
    def load_config(self, config_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if az_org in ("your-organization-name", "ORG_NAME_PLACEHOLDER"):
                raise ValueError(
                    "Azure DevOps organization not set. Update 'azure_devops.organization' in config.json or provide AZURE_DEVOPS_ORGANIZATION in .env and reference it as ${AZURE_DEVOPS_ORGANIZATION}."
                )
    ...
    def _detect_unresolved(self, obj) -> set:
        ...
        if isinstance(obj, dict):
            ...
            isinstance(obj, str)
            and obj.startswith("[PLACEHOLDER_")
            and obj.endswith("]")
        ):
    ...
    def migrate_repository(
        ...
        try:
            self.logger.info(
                f"{'[DRY RUN] ' if dry_run else ''}Starting migration of repository '{repo_name}' from project '{project_name}'"
            )
            ...
            if any(c.isspace() for c in github_repo_name):
                ...
                if normalized != github_repo_name:
                    self.logger.info(
                        f"Auto-normalized target GitHub repository name '{github_repo_name}' -> '{normalized}' (whitespace to underscore)"
                    )
            ...
            if not self._validate_repository_prerequisites(
                project_name, repo_name, dry_run
            ):
                return False
            ...
            azure_data = self.azure_client.export_repository_data(
                ...
                pipeline_scope=(
                    self.config.get("pipelines", {}).get(
                        "scope", getattr(self.args, "pipelines_scope", "project")
                    )
                    if hasattr(self, "args")
                    else "project"
                ),
                exclude_disabled_pipelines=(
                    self.config.get("pipelines", {}).get("exclude_disabled", False)
                    or getattr(self.args, "exclude_disabled_pipelines", False)
                    if hasattr(self, "args")
                    else False
                ),
            )
            ...
            github_repo = self._setup_github_repository(
                azure_data, github_repo_name, dry_run
            )
            ...
            if hasattr(self, "args") and getattr(self.args, "no_git", False):
                skip_git = True
                self.logger.info(
                    "[SKIP] Git history migration skipped due to --no-git flag"
                )
            if not skip_git:
                self._update_migration_state("Migrating Git history")
                if not self._migrate_git_repository(
            ...
            if migrate_pipelines and azure_data.get("pipelines"):
                ...
                self._migrate_pipelines(
                    azure_data["pipelines"], github_repo_name, dry_run
                )
            ...
            if migrate_issues and azure_data.get("work_items"):
                ...
                self._migrate_work_items_to_issues(
                    azure_data["work_items"], github_repo_name, dry_run
                )

            # Step 7: Generate migration report
            self._update_migration_state("Generating migration report")
            self._save_enhanced_migration_report(
                project_name, repo_name, azure_data, start_time, dry_run
            )

            duration = datetime.now() - start_time
            self.logger.info(
                f"[OK] {'[DRY RUN] ' if dry_run else ''}Successfully completed migration of '{repo_name}' in {duration}"
            )
            ...
            duration = datetime.now() - start_time
            self.logger.error(
                f"[ERROR] Failed to migrate repository '{repo_name}' after {duration}: {str(e)}"
            )
            self.migration_state["failed_steps"].append(
                self.migration_state["current_step"]
            )
            return False
        finally:
    ...
    def _validate_repository_prerequisites(
        self, project_name: str, repo_name: str, dry_run: bool
    ) -> bool:
        ...
        try:
            ...
            if repo_size > 5 * 1024 * 1024 * 1024:  # 5GB warning
                self.logger.warning(
                    f"Repository is large ({repo_size / 1024 / 1024:.1f}MB). Migration may take significant time."
                )
    ...
    def _setup_github_repository(
        self, azure_data: Dict[str, Any], github_repo_name: str, dry_run: bool
        ...
        try:
            ...
            if repo_exists:
                self.logger.info(
                    f"GitHub repository '{github_repo_name}' already exists"
                )
                if dry_run:
                    return {
                        "name": github_repo_name,
                        "html_url": f"https://github.com/{github_repo_name}",
                    }
                return self.github_client.get_repository(github_repo_name)

            if dry_run:
                self.logger.info(
                    f"[DRY RUN] Would create GitHub repository: {github_repo_name}"
                )
                return {
                    "name": github_repo_name,
                    "html_url": f"https://github.com/{github_repo_name}",
            ...
            self.logger.info(
                f"[OK] Created GitHub repository: {github_repo['html_url']}"
            )
            return github_repo
    ...
    def _migrate_git_repository(
        self, project_name: str, repo_name: str, github_repo_name: str, dry_run: bool
    ) -> bool:
    ...
    def _migrate_pipelines(
        self, pipelines: List[Dict[str, Any]], github_repo_name: str, dry_run: bool
    ):
        ...
        try:
            ...
            try:
                allow_local = getattr(self, "args", None) and getattr(
                    self.args, "allow_local_workflows", False
                )
            except Exception:
                allow_local = False
            cwd = os.getcwd()
            ...
            if is_tool_repo and not allow_local:
                self.logger.info(
                    "[GUARDRAIL] Skipping local workflow file emission (use --allow-local-workflows to override). Workflows will only be pushed to target repository."
                )
            ...
            if converted_files:
                self.logger.info(
                    f"[OK] Converted {len(converted_files)} pipelines to GitHub Actions (isolated)"
                )
                if not dry_run:
                    try:
                        ...
                        self.logger.info(
                            f"Preparing to commit workflows to {repo_url} from isolated directory"
                        )
                        ...
                        if clone_result.returncode != 0:
                            self.logger.warning(
                                f"Could not clone repo to add workflows: {clone_result.stderr.strip()}"
                            )
                        else:
                            dest_wf_dir = os.path.join(
                                temp_clone_dir, ".github", "workflows"
                            )
                            ...
                            for wf in converted_files:
                                ...
                                if os.path.exists(dst_path):
                                    ...
                                    dst_path = os.path.join(
                                        dest_wf_dir, f"{base}-migrated{ext}"
                                    )
                                    self.logger.warning(
                                        f"Workflow {wf} already exists in target repo – saving as {os.path.basename(dst_path)}"
                                    )
                            ...
                            if changes:
                                ...
                                if (
                                    ...
                                    git_user_email = os.environ.get(
                                        "GIT_USER_EMAIL",
                                        "azuredevops-github-migration[bot]@users.noreply.github.com",
                                    )
                                    git_user_name = os.environ.get(
                                        "GIT_USER_NAME", "ADO-GH Migration Tool"
                                    )
                                    subprocess.run(
                                        [
                                            "git",
                                ...
                                if add_res.returncode != 0:
                                    self.logger.warning(
                                        f"Failed to stage workflows: {add_res.stderr.strip()}"
                                    )
                                else:
                                    commit_res = subprocess.run(
                                        [
                                            ...
                                            "commit",
                                            "-m",
                                            "Add converted Azure DevOps pipelines as GitHub Actions workflows",
                                        ],
                                        capture_output=True,
                                        text=True,
                                    )
                                    if commit_res.returncode == 0:
                                        push_res = subprocess.run(
                                            ["git", "-C", temp_clone_dir, "push"],
                                            capture_output=True,
                                            text=True,
                                        )
                                        if push_res.returncode == 0:
                                            self.logger.info(
                                                "[OK] Workflow files committed and pushed to GitHub"
                                            )
                                        else:
                                            self.logger.warning(
                                                f"Failed to push workflows: {push_res.stderr.strip()}"
                                            )
                                    else:
                                        combined = (
                                        ...
                                        if "nothing to commit" in combined:
                                            self.logger.info(
                                                "No workflow changes to commit (already present)"
                                            )
                                        else:
                                            self.logger.warning(
                                                f"Failed to commit workflows: {commit_res.stderr.strip()}"
                                            )
                    except Exception as wf_err:
                        self.logger.warning(
                            f"Could not auto-commit workflows: {wf_err}"
                        )
    ...
    def _migrate_work_items_to_issues(
        self,
        work_items: List[Dict[str, Any]],
        github_repo_name: str,
        dry_run: bool = False,
        ...
        self.logger.info(
            f"{'[DRY RUN] ' if dry_run else ''}Migrating {len(work_items)} work items to GitHub issues"
        )
        ...
        with tqdm(
            total=len(work_items), desc="Migrating work items", unit="item"
        ) as pbar:
            for work_item in work_items:
                try:
                    ...
                    labels = [
                        f"migrated",
                        f"type:{work_item_type.lower()}",
                        f"state:{state.lower()}",
                    ]

                    issue = self.github_client.create_issue(
                        github_repo_name, title, body, labels
                    )

                    self.logger.debug(
                        f"Created issue #{issue['number']} for work item #{work_item['id']}"
                    )
                    pbar.update(1)

                except Exception as e:
                    self.logger.error(
                        f"Failed to migrate work item #{work_item['id']}: {str(e)}"
                    )
                    pbar.update(1)

    # Keep the old method name for backward compatibility
    def migrate_work_items_to_issues(
        self, work_items: List[Dict[str, Any]], github_repo_name: str
    ):
    ...
    def _save_enhanced_migration_report(
        self,
        project_name: str,
        repo_name: str,
        ...
        if hasattr(self, "git_migrator") and getattr(
            self.git_migrator, "last_remote_verification", None
        ):
        ...
        report = {
            ...
            "migration_statistics": {
                ...
                and not (hasattr(self, "args") and getattr(self.args, "no_git", False)),
                "pipelines_converted": len(azure_data.get("pipelines", [])) > 0,
            },
            "verification": remote_verification,
            "detailed_data": (
                azure_data
                if self.config.get("output", {}).get("save_raw_data", False)
                else None
            ),
        }
        ...
        report_file = os.path.join(
            output_dir, f"migration_report_{project_name}_{repo_name}_{timestamp}.json"
        )
    ...
    def _create_summary_report(
        self, full_report: Dict[str, Any], output_dir: str, timestamp: str
    ):
        """Create a human-readable summary report."""
        try:
            summary_file = os.path.join(
                output_dir, f"migration_summary_{timestamp}.txt"
            )
            ...
            with open(summary_file, "w", encoding="utf-8") as f:
                ...
                f.write(
                    f"Source: {source['organization']}/{source['project']}/{source['repository']}\n"
                )
                ...
                f.write("-" * 20 + "\n")
                f.write(
                    f"Repository Size: {stats['repository_size_bytes'] / 1024 / 1024:.1f} MB\n"
                )
                ...
                f.write(
                    f"Git History Migrated: {'Yes' if stats['git_history_migrated'] else 'No'}\n"
                )
                f.write(
                    f"Pipelines Converted: {'Yes' if stats['pipelines_converted'] else 'No'}\n\n"
                )
    ...
    def save_migration_report(
        self, project_name: str, repo_name: str, azure_data: Dict[str, Any]
    ):
        """Legacy method - use _save_enhanced_migration_report instead."""
        return self._save_enhanced_migration_report(
            project_name, repo_name, azure_data, datetime.now(), False
        )

    def migrate_multiple_repositories(
...
def main(argv=None):
  ...
  python migrate.py --project "MyProject" --repo "my-repo" --no-issues --no-pipelines
    ...
    parser.add_argument(
        "--config", default="config.json", help="Configuration file path (JSON or YAML)"
    )
    parser.add_argument("--debug", action="store_true", help="Enable debug logging")

    # Migration targets
    parser.add_argument("--project", help="Azure DevOps project name")
    parser.add_argument("--repo", help="Repository name to migrate")
    parser.add_argument(
        "--github-repo", help="GitHub repository name (defaults to source repo name)"
    )
    ...
    parser.add_argument(
        "--no-issues", action="store_true", help="Skip migrating work items as issues"
    )
    parser.add_argument(
        "--no-pipelines",
        action="store_true",
    ...
    parser.add_argument("--list-repos", help="List repositories in specified project")
    parser.add_argument(
        "--list-pipelines",
    ...
    try:
        ...
        if (
            args.list_projects
            or args.list_repos
            or args.list_pipelines
            or args.list_pipelines_repo
        ):
        ...
        if not orchestrator.validate_credentials():
            print(
                "[ERROR] Credential validation failed. Please check your configuration."
            )
            exit(1)
        ...
        try:
            if (
                migrate_issues_flag
                and orchestrator.config.get("migration", {}).get("migrate_work_items")
                is False
            ):
        ...
        if success:
            print(
                f"[OK] {'[DRY RUN] ' if args.dry_run else ''}Migration completed successfully!"
            )
            if args.dry_run:
...
def handle_list_commands(args) -> int:
    ...
    try:
        ...
        if args.list_repos:
            ...
            print("-" * 50)
            for repo in repos:
                size = orchestrator.azure_client.get_repository_size(
                    args.list_repos, repo["id"]
                )
        ...
        if args.list_pipelines:
            ...
            for p in pipelines:
                ...
                print(
                    f"• {name} (id={pid}) queueStatus={qs}{' [DISABLED]' if disabled else ''}"
                )
            return 0
        ...
        if args.list_pipelines_repo:
            ...
            if not repo:
                print(
                    f"[ERROR] Repository '{repo_name}' not found in project '{project}'"
                )
                return 1
            pipelines = orchestrator.azure_client.get_pipelines_for_repo(
                project, repo["id"]
            )
            print(
    ...
    try:
        ...
        try:
            ...
            if azure_valid:
                projects = orchestrator.azure_client.get_projects()
                print(
                    f"[OK] Azure DevOps: Connected successfully ({len(projects)} projects accessible)"
                )
            else:
        ...
        try:
            ...
            if github_valid:
                ...
                print(
                    f"[OK] GitHub: Connected as {user['login']} ({remaining} API calls remaining)"
                )
            else:
