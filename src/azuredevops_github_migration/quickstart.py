...
def main(argv=None):
    ...
    parser.add_argument(
        "--skip-init", action="store_true", help="Skip init even if config.json missing"
    )
    parser.add_argument(
        "--skip-env",
        action="store_true",
        help="Skip interactive environment variable entry",
    )
    parser.add_argument(
        "--no-analyze", action="store_true", help="Skip analyze listing step"
    )
    parser.add_argument(
        "--non-interactive",
        action="store_true",
        help="Do not prompt for confirmation between steps",
    )
    parser.add_argument(
        "--no-project-select",
        action="store_true",
        help="Disable interactive project selection even if questionary is installed",
    )
    parser.add_argument(
        "--open-menu",
        action="store_true",
        help="Launch interactive menu after quickstart completes (bypasses prompt)",
    )
    parser.add_argument(
        "--debug", action="store_true", help="Verbose logging (passes through)"
    )
    ...
    try:
        if os.path.exists("config.json"):
            ...
            placeholder_tokens = {
                ...
                "your-github-org",
                "your_azure_devops_org_here",
                "your_github_org_here",
            }
            env_ad = os.getenv("AZURE_DEVOPS_ORGANIZATION") or os.getenv(
                "AZURE_DEVOPS_ORG"
            )
            ...
            if ad_org in placeholder_tokens and env_ad:
                ...
                changed = True
                _print(
                    f"Patched azure_devops.organization -> {env_ad} (from environment)"
                )
            ...
            if changed:
                ...
                try:
                    ...
                    _print(
                        f"Updated config.json (backup saved as config.json.bak.{ts})"
                    )
    ...
    if not args.skip_env:
        if _confirm("environment variable entry (doctor --edit-env)"):
            try:
                ...
                _print(
                    "Launching interactive environment editor (doctor --edit-env)..."
                )
                rc = doctor_main(["--edit-env"])
                if rc not in (
                    0,
                    1,
                ):  # editor returns 0 normally; 1 may signify missing tokens yet
    ...
    try:
        ...
        if rc != 0:
            _print(
                "Doctor reported issues (see above). You may continue, but address critical errors first."
            )
    except Exception as e:
        ...
        try:
            ...
            try:
                analyzer = AzureDevOpsAnalyzer(
                    "config.json", skip_work_items=True, omit_work_item_fields=True
                )
                projects = analyzer.client.get_projects()
                project_names = sorted(
                    [p["name"] for p in projects], key=lambda s: s.lower()
                )
                ...
                if can_select and project_names:
                        ...
                        try:
                            ...
                            while True:
                                ...
                                if total == 0:
                                    ...
                                    if ans == "New search":
                                        s = questionary.text(
                                            "Search projects (substring or fuzzy):"
                                        ).ask()
                                        if s:
                                            filter_text = s
                                ...
                                ans = questionary.select(
                                    prompt + ":", choices=q_choices, qmark="📁"
                                ).ask()
                    ...
                    if (
                        ...
                        _print(
                            "(Install 'questionary' for interactive project selection: pip install questionary)"
                        )
    ...
    if selected_project:
        _print(
            f"  1) Analyze selected project: azuredevops-github-migration analyze --project {proj_token_disp} --create-plan"
        )
        _print(
            "     (Full org analysis: azuredevops-github-migration analyze --create-plan)"
        )
    else:
        _print(
            "  1) Analyze a single project (replace <Project>): azuredevops-github-migration analyze --project <Project> --create-plan"
        )
        _print(
            "     (Full org analysis is heavier; run only when needed: azuredevops-github-migration analyze --create-plan)"
        )
    _print(
        f"  2) Dry run a repository: azuredevops-github-migration migrate --project {proj_token_disp} --repo <Repo> --dry-run --config config.json"
    )
    _print(
        f"  3) Real migration: azuredevops-github-migration migrate --project {proj_token_disp} --repo <Repo> --config config.json"
    )
    _print(
        "  4) Batch (when ready): azuredevops-github-migration batch --plan migration_plan_<org>_*.json"
    )
    if selected_project:
        _print(
            f"(Project '{selected_project}' selected for examples. Use --no-project-select to suppress.)"
        )
