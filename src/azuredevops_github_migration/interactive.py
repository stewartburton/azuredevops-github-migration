...
def run_update_env(path: str = ".env") -> int:
    ...
    script_relative = os.path.join(
        os.path.dirname(__file__), "..", "..", "scripts", "Test-MigrationEnv.ps1"
    )
    ...
    if not os.path.exists(path):
        with open(path, "w", encoding="utf-8") as f:
            f.write(
                "# Auto-created .env stub -- fill in values. Never commit real secrets.\n"
            )
        ...
        print(
            f"Created stub {path}. Populate credential and organization values, then re-run update-env."
        )

    shell_cmd = _find_powershell()
    if not shell_cmd:
        print(
            "No PowerShell host available (pwsh / powershell). Install PowerShell 7+ for this feature."
        )
        return 1
...
def _gather_readiness() -> Dict[str, any]:
    ...
    readiness_level = (
        "READY"
        if (cfg_exists and env_exists and all_present and not any_placeholders)
        else "INCOMPLETE"
    )
    return {
        "config": cfg_exists,
...
def _print_readiness_banner():
    ...
    for k in (
        ...
        state = (
            ...
            else ("PH" if v["placeholder"] else "MISSING")
        )
...
def interactive_menu():
    ...
    if not questionary:
        print(
            "Optional dependency 'questionary' not installed. Install with: pip install questionary"
        )
        return 1
    color_disabled = bool(os.environ.get("NO_COLOR"))
    ...
    if not color_disabled:
        print(
            "(Interactive Menu) — Icons indicate action category. Set MIGRATION_CLI_NO_ICONS=1 to disable icons."
        )
    ...
    while True:
        ...
        if key == "init":
            subprocess.run(
                [sys.executable, "-m", "azuredevops_github_migration", "init"]
            )
        elif key == "migrate":
            ...
            try:
                ...
                if len(project_names) == 1:
                    ...
                    project = _paginated_picker(
                        "Select source project", project_names, allow_cancel=True
                    )
                if not project:
                    continue
                ...
                if len(repo_names) == 1:
                    ...
                    repo = _paginated_picker(
                        f"Select repository in {project}", repo_names, allow_cancel=True
                    )
                if not repo:
                    continue
                # Ask dry run or real
                ...
                gh_custom = questionary.text(
                    "GitHub repository name (leave blank to use same name):"
                ).ask()
                ...
                print(
                    f"Interactive migrate flow failed; falling back to basic migrate: {e}"
                )
                subprocess.run(
                    [sys.executable, "-m", "azuredevops_github_migration", "migrate"]
                )
        elif key == "analyze":
            ...
            try:
                choice = questionary.select(
                    ...
                    choices=[
                        questionary.Choice(
                            title="Single project (faster)", value="single"
                        ),
                        questionary.Choice(
                            title="Full organization (all projects & repos)",
                            value="full",
                        ),
                        questionary.Choice(title="Cancel", value="cancel"),
                    ],
                ...
                subprocess.run(
                    [sys.executable, "-m", "azuredevops_github_migration", "analyze"]
                )
        elif key == "batch":
            subprocess.run(
                [sys.executable, "-m", "azuredevops_github_migration", "batch"]
            )
            ...
            sub = questionary.select(
                ...
                choices=[
                    ...
                    questionary.Choice(
                        title="Enter / update environment variables (--edit-env)",
                        value="edit_env",
                    ),
                    questionary.Choice(
                        title="Append missing env placeholders (doctor --fix-env)",
                        value="fix",
                    ),
                    questionary.Choice(
                        title="Remediation assistant (--assist)", value="assist"
                    ),
            ...
            if sub == "plain":
                subprocess.run(
                    [sys.executable, "-m", "azuredevops_github_migration", "doctor"]
                )
            elif sub == "fix":
                subprocess.run(
                    [
...
def _paginated_picker(
    ...
    filter_text = ""
    page = 0
    use_icons = os.getenv("MIGRATION_CLI_NO_ICONS", "").lower() not in (
        "1",
        "true",
        "yes",
    )
    ...
    while True:
        ...
        if total == 0:
            ...
            ans = questionary.select(
                f"No results for '{filter_text}'.", choices=q_choices, qmark="📁"
            ).ask()
            if ans == clear_label:
                filter_text = ""
                filtered = full_list
        ...
        if ans == jump_label:
            ...
            if letter:
                l = letter[0].lower()
                idx = next(
                    (i for i, n in enumerate(filtered) if n.lower().startswith(l)), None
                )
                if idx is not None:
                    page = idx // page_size
            continue
