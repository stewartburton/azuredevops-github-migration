...
def test_interactive_analyze_injects_skip(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(mod, 'questionary', types.SimpleNamespace(select=lambda *a, **k: None))
    ...
    try:
        ...
        fake_run([sys.executable, '-m', 'azuredevops_github_migration', 'analyze', *skip_flag])
    finally:
        pass

    assert '--skip-work-items' in called.get('cmd', []), f"Expected --skip-work-items in analyze cmd: {called.get('cmd')}"
