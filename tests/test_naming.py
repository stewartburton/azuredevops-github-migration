...
def test_workflow_long_truncation_and_suffix():
    seen = set()
    long_name = "X" * 120
    stem1 = naming.normalize_workflow_stem(
        long_name, seen, {"naming": {"workflow": {"max_length": 10}}}
    )
    stem2 = naming.normalize_workflow_stem(
        long_name, seen, {"naming": {"workflow": {"max_length": 10}}}
    )
    assert len(stem1) <= 10
    assert stem2 != stem1
...
def test_repo_whitespace_dash():
    name = naming.normalize_repo_name(
        "My Repo Name", {"naming": {"repository": {"whitespace_strategy": "dash"}}}
    )
