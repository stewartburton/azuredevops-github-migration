...
class TestNoGitFlag(unittest.TestCase):
    ...
    @patch(
        "azuredevops_github_migration.migrate.MigrationOrchestrator._migrate_git_repository"
    )
    @patch(
        "azuredevops_github_migration.migrate.MigrationOrchestrator._setup_github_repository"
    )
    @patch(
        "azuredevops_github_migration.migrate.AzureDevOpsClient.export_repository_data"
    )
    @patch(
        "azuredevops_github_migration.migrate.MigrationOrchestrator._validate_repository_prerequisites"
    )
    def test_no_git_skips_git_history(
        self, mock_validate, mock_export, mock_setup, mock_git
    ):
        ...
        orch.args = Args()
        ok = orch.migrate_repository(
            "proj", "repo", migrate_issues=False, migrate_pipelines=False, dry_run=True
        )
        self.assertTrue(ok)
