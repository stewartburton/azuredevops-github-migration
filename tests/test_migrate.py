...
class TestAzureDevOpsClient(unittest.TestCase):
    ...
    def test_get_projects_success(self, mock_get):
        resp = Mock(); resp.json.return_value={'value':[{'name':'P1'}]}; resp.raise_for_status.return_value=None; mock_get.return_value=resp
...
class TestGitMigrator(unittest.TestCase):
    def test_add_auth_to_url(self):
        migrator = GitMigrator(Mock(spec=AzureDevOpsClient), Mock(spec=GitHubClient), Mock())
        self.assertEqual(
...
class TestPipelineConverter(unittest.TestCase):
    ...
    class TestMigrationOrchestrator(unittest.TestCase):
        ...
        def test_init(self, mock_gh, mock_ado):
            ...
            json.dump({'azure_devops': {'organization':'o','personal_access_token':'p'}, 'github': {'token':'t'}}, tmp); tmp.close()
            try:
                orch = MigrationOrchestrator(tmp.name)
                self.assertIsNotNone(orch.config)
                mock_ado.assert_called_once(); mock_gh.assert_called_once()
            finally:
                os.unlink(tmp.name)
